...
import com.atguigu.gmall.bean.{Category3, SkuInfo, SpuInfo}
...
object DimSpuInfo {
  def main(args: Array[String]): Unit = {
    ...
    jsonObjDstream.foreachRDD { rdd =>
      // xxxx driver
      val skuRDD: RDD[SpuInfo] = rdd.map { jsonObj => //ex
        val spuinfo: SpuInfo = SpuInfo(jsonObj.getString("id"),
      ...
      //保存到HBase
      skuRDD.saveToPhoenix("GMALL_SPU_INFO",
        Seq("ID", "SPUNAME"), new Configuration,
